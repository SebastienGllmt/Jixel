...
public class JixelScreen extends Canvas {
	...
	private void moveCamera(int xOffset, int yOffset, int lockedX, int lockedY) {
		int diffX = lockedX - xOffset;
		int diffY = lockedY - yOffset;
		setOffset((int) (xOffset + Integer.signum(diffX) * Math.sqrt(Math.abs(diffX))), (int) (yOffset + Integer.signum(diffY) * Math.sqrt(Math.abs(diffY))));
	}
	...
	public void updateCamera(int xOffset, int yOffset) {
		if (lockedEntity != null) {
			...
			moveCamera(xOffset, yOffset, x, y);
		}
	}
